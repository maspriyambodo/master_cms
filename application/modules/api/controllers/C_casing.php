...
class C_casing extends CI_Controller {
    ...
    public function Set_casing() {
        $curl = curl_init();
        curl_setopt_array($curl, [
            CURLOPT_URL => base_url('data_enterkomputer/casing.json'),
        ...
        for ($index = 0; $index < $tot_data; $index++) {
            for ($index2 = 0; $index2 < count($jso['PPRNT'][$index]['PCHLD'][0]['PLIST']); $index2++) {
                $data[] = [
                    ...
                    'form_factor' => $jso['PPRNT'][$index]['PCHLD'][0]['PCHLD'],
...
//        print_array($data2);
        $this->db_pc->insert_batch('dt_casing', $data2);
        echo 'update success';
    }
}
